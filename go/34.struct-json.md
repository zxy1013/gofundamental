...
func timeFieldDemo() {
	...
	fmt.Printf("str:%s\n", b) // str:{"create_time":"2020-04-05T12:28:06.799214+08:00"}
...
也就是内置的json包不识别我们常用的字符串时间格式，如`2020-04-05 12:25:42`。不过我们可以通过实现 `json.Marshaler`/`json.Unmarshaler` 接口实现自定义的事件格式解析。
...
const ctLayout = "2006-01-02 15:04:05" // 标准格式
...
如果json串没有固定的格式导致不好定义与其相对应的结构体时，我们可以使用`json.RawMessage`将原始字节数据保存下来。
