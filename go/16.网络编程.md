...
电脑要与外界互联网通信，需要先把电脑连接网络，我们可以用双绞线、光纤、无线电波等方式。这就叫做”实物理层”，它就是把电脑连接起来的物理手段。它主要规定了网络的一些电气特性，作用是负责传送0和1的电信号。
...
“端口”是0到65535之间的一个整数，正好16个二进制位。0到1023的端口被系统占用，用户只能选用大于1023的端口。有了IP和端口就能实现唯一确定互联网上一个程序，进而实现网络间的程序通信。
...
一个TCP服务端可以同时连接很多个客户端，例如世界各地的用户使用自己电脑上的浏览器访问淘宝网。因为Go语言中创建多个goroutine实现并发非常方便和高效，所以可以每建立一次链接就创建一个goroutine去处理。
...
3. 创建goroutine处理链接。
   ...
   func process(conn net.Conn) {
   	defer conn.Close() // 处理完毕后 关闭连接
   	// 针对当前连接做数据处理或接收操作
   	for {
   		reader := bufio.NewReader(conn) // 创建读对象
   		var buf [128]byte
   		n, err := reader.Read(buf[:]) // 读取数据 n表示长度
   		if err != nil {
   			fmt.Println("read from client failed, err:", err)
   			break
   		}
   		recvStr := string(buf[:n]) // 强转
   		fmt.Println("收到client端发来的数据：", recvStr)
   		conn.Write([]byte(recvStr)) // 强转类型后 发送给客户端数据
   	}
   }
   
   func main() {
   	listen, err := net.Listen("tcp", "127.0.0.1:20000") // 开启服务 TCP协议 ip端口
   	if err != nil {
   		fmt.Println("listen failed, err:", err)
   		return
   	}
   	for {
   		conn, err := listen.Accept() // 等待客户端建立连接 监听端口 阻塞 等待连接
   		if err != nil {
...
func main() {
	...
	defer conn.Close() // 操作完成后 关闭连接
	// 利用该连接 进行数据收发
	inputReader := bufio.NewReader(os.Stdin) // 从标准输入创建读对象
	for {
		input, _ := inputReader.ReadString('\n') // 读取用户输入 读一行
		inputInfo := strings.Trim(input, "\r\n") // 去空格
		if strings.ToUpper(inputInfo) == "Q" { // 转化为大写 如果输入q就退出
			return
		}
...
> go build
>
> 生成两个exe文件， 先启动server端再启动client端，在client端输入任意内容回车之后就能够在server端看到client端发送的数据，从而实现TCP通信。 
>
> 启动：可以在文件夹打开exe 也可以在tcpserver目录下的终端输入.\Tserver.exe ，tcpclient目录下的终端输入 .\Tclient.exe

**TCP黏包**

**为什么会出现粘包**

主要原因就是tcp数据传递模式是流模式，在保持长连接的时候可以进行多次的收和发。所以每次传输的边界无法判断
...
出现”粘包”的关键在于接收方不确定将要传输的数据包的大小，因此我们可以对数据包进行封包和拆包的操作。封包：封包就是给一段数据加上包头，这样一来数据包就分为包头和包体两部分内容了(过滤非法包时封包会加入”包尾”内容)。包头部分的长度是固定的，并且它存储了包体的长度，根据包头长度固定以及包头中含有包体长度的变量就能正确的拆分出一个完整的数据包。
...
将上面的代码保存后，分别编译。先启动服务端再启动客户端，可以看到客户端分10次发送的数据，在服务端并没有成功的输出10次，而是多条数据“粘”到了一起。
...
func Encode(message string) ([]byte, error) {
	...
	// 写入消息头
	err := binary.Write(pkg, binary.LittleEndian, length) // int型 二进制存
	if err != nil {
		return nil, err
	}
	// 写入消息实体
...
func Decode(reader *bufio.Reader) (string, error) {
	...
	var length int32
	err := binary.Read(lengthBuff, binary.LittleEndian, &length) // 读取消息的长度int
	if err != nil {
		return "", err
	}
...
func main() {
	...
	defer socket.Close()
	input := bufio.NewReader(os.Stdin) // 从标准输入创建读对象
	for {
	sendData, _ := input.ReadString('\n') // 读取用户输入 读一行
...
> 先启动server端再启动client端，在client端输入任意内容回车之后就能够在server端看到client端发送的数据，从而实现UDP通信。 