...
此外，一些厂商公开的API服务通常也会限制用户的请求次数，比如百度地图开放平台等会根据用户的付费情况来限制用户的请求数等。 ![百度地图开放平台API调用策略](F:\markdown笔记\go/baidumap_ratelimit.jpg)
...
![漏桶算法原理](F:\markdown笔记\go/loutong.jpg)

漏桶法的关键点在于漏桶始终按照固定的速率运行，但是它并不能很好的处理有大量突发请求的场景，毕竟在某些场景下我们可能需要提高系统的处理效率，而不是一味的按照固定速率处理请求。![微信图片_20220105172819](F:\markdown笔记\go\微信图片_20220105172819.jpg)
...
type limiter struct {
	sync.Mutex                // 锁
	last       time.Time      // 上一次完成的时刻 at
	sleepFor   time.Duration  // 需要等待的时间
	perRequest time.Duration  // 每次处理的时间间隔
...
![令牌桶原理](F:\markdown笔记\go/lingpaitong.jpg)

对于从桶里取不到令牌的场景，我们可以选择等待也可以直接拒绝并返回。

![微信图片_20220105172839](F:\markdown笔记\go\微信图片_20220105172839.jpg)
...
// 创建填充速度为指定速率和容量大小的令牌桶 每秒产生 rate 速率的令牌 
